...
app.get("/video", (req, res) => {
    ...
    console.log('Streaming video from path ${path}.');

    const blobService = createBlobService();

    const containerName = "video-streaming";
    blobService.getBlobProperties(containerName, path, (err, properties) => {
        if(err) {
            console.error(`Error occurred getting properties for video ${containerName}/${path}.`);
        ...
        blobService.getBlobToStream(containerName, path, res, err => {
            if(err) {
                console.error(`Error occurred getting video ${containerName}/${path} to stream.`);
                console.error(err && err.stack || err);
                res.sendStatus(500);
                return;
            }
